#!/usr/bin/env python3
"""Package Warm Sand Deconstructed Trousers canonical sitting candidate."""
...
MASTER = EVIDENCE / "item-masters/warm-sand-deconstructed-trousers-sitting-master-v1-1024.png"
OUTPUT = EVIDENCE / (
    "candidates/room_avatar_bottom_male_warm_sand_deconstructed_trousers_v1_"
    "sitting_front_f01-candidate-v1.png"
)
COMPOSITE = EVIDENCE / "warm-sand-deconstructed-trousers-canonical-sitting-v1.png"
REVIEW_BOARD = EVIDENCE / "warm-sand-deconstructed-trousers-sitting-v1-review-board.png"
MANIFEST = EVIDENCE / "warm-sand-deconstructed-trousers-sitting-v1-manifest.json"
...
def extract_bottom(master: Image.Image) -> Image.Image:
    ...
    # The outer arm zones share the warm palette; geometry is the reliable
    # separator on this fixed canonical sitting base.
    side_arm_zone = (
        (rows_4x >= 280 * SCALE)
        & (rows_4x <= 326 * SCALE)
        & ((cols_4x <= 89 * SCALE) | (cols_4x >= 167 * SCALE))
    )
    material = in_zone & warm_sand & ~side_arm_zone
...
def review_board(bottom: Image.Image, composite: Image.Image) -> Image.Image:
    ...
    draw.text(
        (28, 18),
        "WARM SAND DECONSTRUCTED TROUSERS · SITTING V1",
        font=_font(28, True),
        fill="#382c37",
    )
    draw.text(
        (28, 56),
        "item-specific warm material · soft hem contact · runtime promotion closed",
        font=_font(18),
        fill="#796976",
    )
    ...
    draw.text((30, 550), "5x WAIST / SOFT THIGHS", font=_font(18), fill="#382c37")
...
def _write_manifest() -> None:
    payload = {
        ...
        "assetId": "blumi-avatar-bottom-warm-sand-deconstructed-trousers-sitting-v1.0",
        "itemId": "warm_sand_deconstructed_trousers",
        "fitFamily": "male_soft_straight",
        ...
        "evidence": {
            ...
            "focusedTest": "python3 -m unittest test_package_male_sitting_warm_sand_deconstructed_v1.py",
        },
        "continuityLocks": {
