#!/usr/bin/env python3
"""Package Midnight Relaxed Tailoring Trousers sitting candidate."""
...
MASTER = EVIDENCE / "item-masters/midnight-relaxed-tailoring-trousers-sitting-master-v1-1024.png"
OUTPUT = EVIDENCE / (
    "candidates/room_avatar_bottom_male_midnight_relaxed_tailoring_trousers_v1_"
    "sitting_front_f01-candidate-v1.png"
)
COMPOSITE = EVIDENCE / "midnight-relaxed-tailoring-trousers-canonical-sitting-v1.png"
REVIEW_BOARD = EVIDENCE / "midnight-relaxed-tailoring-trousers-sitting-v1-review-board.png"
MANIFEST = EVIDENCE / "midnight-relaxed-tailoring-trousers-sitting-v1-manifest.json"
GARMENT_ZONE = (74, 270, 182, 345)
...
def extract_bottom(master: Image.Image) -> Image.Image:
    ...
    material = in_zone & midnight
    isolated = np.zeros_like(source)
    isolated[material] = source[material]
    ...
    registered[334:] = 0
    # The generated master uses a white staging background between the legs.
    # On the real layered avatar that hole exposes orange base-leg pixels and
    # reads as a tear. Reconstruct a narrow opaque tailored seam through the
    # pelvis; true separation starts only at the shoe-tongue opening below.
    for row in range(306, 329):
        left_sample = registered[row, 122:126, :3].astype(np.float32)
        right_sample = registered[row, 131:135, :3].astype(np.float32)
        bridge_rgb = np.rint(np.concatenate((left_sample, right_sample)).mean(axis=0) * 0.92)
        seam_rgb = np.rint(bridge_rgb * 0.55)
        registered[row, 126:131, :3] = bridge_rgb.astype(np.uint8)
        registered[row, 126:131, 3] = 255
        registered[row, 128, :3] = seam_rgb.astype(np.uint8)
...
def review_board(bottom: Image.Image, composite: Image.Image) -> Image.Image:
    ...
    draw.text(
        (28, 18), "MIDNIGHT RELAXED TAILORING · SITTING V1",
        font=_font(28, True), fill="#382c37",
    )
    draw.text(
        (28, 56), "relaxed seated volume · progressive shoe contact · runtime closed",
        font=_font(18), fill="#796976",
    )
    ...
    contact = composite.crop((82, 321, 174, 350)).resize((552, 174), Image.Resampling.NEAREST)
    board.paste(waist, (30, 585), waist)
    board.paste(contact, (540, 590), contact)
    draw.text((30, 550), "5x WAIST / RELAXED THIGHS", font=_font(18), fill="#382c37")
...
def _write_manifest() -> None:
    payload = {
        ...
        "assetId": "blumi-avatar-bottom-midnight-relaxed-tailoring-sitting-v1.0",
        ...
        "source": {
            ...
            "modelOrEngine": "not_provided", "requestOrJobId": "not_provided",
            "seed": "not_provided",
        },
        "candidate": {
        ...
        "evidence": {
            ...
            "focusedTest": "python3 -m unittest test_package_male_sitting_midnight_relaxed_v1.py",
        },
        "continuityLocks": {
