...
ROOT = (
    REDESIGN
    / "candidates/bottom/wide_pleated_technical_trousers_v21"
)
GENERATED_ROOT = ROOT / "generated"
OUTPUT_ROOT = ROOT / "motion-v21"
BOARD = ROOT / "wide-pleated-technical-trousers-v21-4w1s-board.png"
MANIFEST = ROOT / "wide-pleated-technical-trousers-v21-motion-manifest.json"
REVIEW = ROOT / "wide-pleated-technical-trousers-v21-independent-review.json"
STATIC_CANDIDATE = (
    REDESIGN
...
def prepare_sources() -> None:
    for state in STATES:
        ...
        dominant = _dominant(pixels[..., 3] > 12)
        pixels[~dominant] = 0
...
def build_frame(state: str) -> Image.Image:
    ...
    width_factor = 0.98 if state == "sitting_front_f01" else 0.94
    target_width = max(
        1,
...
def produce() -> dict:
    ...
    for state, frame in frames.items():
        destination = OUTPUT_ROOT / f"{state}-v21.png"
    ...
    manifest = {
        ...
        "method": (
            ...
            "wide-straight-pleated-canonical-contact-registration"
        ),
        "staticCandidate": {
        ...
        "outputs": {
            _relative(STATIC_CANDIDATE): _sha256(STATIC_CANDIDATE),
        },
        "generatedMasters": {
            state: {
