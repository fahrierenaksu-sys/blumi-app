#!/usr/bin/env python3
"""TDD gate for Midnight Relaxed Tailoring Trousers seated candidate."""
...
SCRIPT = Path(__file__).with_name("package_male_sitting_midnight_relaxed_v1.py")
SPEC = importlib.util.spec_from_file_location("package_midnight_relaxed", SCRIPT)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


class PackageMidnightRelaxedSittingTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for row, col in zip(*np.where(remaining)):
            ...
            while queue:
                ...
                for row_delta, col_delta in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                    ...
                    if 0 <= next_row < mask.shape[0] and 0 <= next_col < mask.shape[1] and remaining[next_row, next_col]:
    ...
    def test_waist_is_continuous_and_inside_top_contact(self) -> None:
    ...
    def test_relaxed_tailoring_keeps_volume_gap_and_midnight_material(self) -> None:
        self.assertGreater(int(self.alpha[316, 88:127].sum()), 28)
        self.assertGreater(int(self.alpha[316, 130:169].sum()), 28)
        # Full-length relaxed trousers must cover the base legs through the
        # pelvis. Separation is a dark tailored seam here, not transparent skin.
        self.assertTrue(self.alpha[306:329, 126:131].all())
        rgb = self.pixels[..., :3].astype(np.int16)
        midnight = self.alpha & (rgb[..., 2] > rgb[..., 0] + 20) & (rgb[..., 2] > rgb[..., 1] + 12)
        self.assertGreater(int(midnight.sum()), 1500)

    def test_relaxed_hems_reveal_shoes_with_progressive_contact(self) -> None:
        shoes = np.asarray(MODULE.load(MODULE.SHOES))[..., 3] > 24
        overlap = self.alpha & shoes
        total = int(overlap.sum())
        self.assertGreater(total, 100)
        self.assertLess(total, 320)
